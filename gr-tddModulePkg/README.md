...
![TDD illustration](image.png)
When the follower is started, it will wait for the first byte to arrive at RX. As in when the first byte is processed, the timer in the Message strobe starts. Later both master and follower will be toggled appropriately between RX and TX according to the UL/DL time specified.
## 2. Analysis
